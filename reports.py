...
def trial_balance(entities: dict[str, root.Entity], accounts: dict[str, root.Account]) -> str:
    ...
    def aux(test: str|None, acc_key: str) -> float:
        return data.consolidate_all(acc_key)[3] if test is None else data.consolidate_ccen(entity, acc_key)[3]
    assets = aux(entity, '1') + aux(entity, '2') + aux(entity, '3')
    results = aux(entity, '4') + aux(entity, '5') + aux(entity, '6')
    oprev = -aux(entity, '4.1')
    oprev = oprev if oprev!=0 else 0.001
    opexp = -aux(entity, '5.1')
    depr = -aux(entity, '5.1.7')
    re = -aux(entity, '4')
    re = re if re!=0 else 0.001
    ex = -aux(entity, '5')
    ex = ex if ex!=0 else -0.001
    ass = aux(entity, '1')
    ass = ass if ass!=0 else 0.001
    x = -aux(entity, '2.1')
    y = aux(entity, '1.1')
    y = y if y!=0 else 0.001
    cash = data.consolidate_ccen(entity, '1.1.1') if entity else data.consolidate_all('1.1.1')
    cash[0] = cash [0] if cash[0]!=0 else 0.001
    debit = x/y
    ...
    fi = 0.001 if fi== 0.002 else fi
    test = 'Loss' if fi < 0 else 'Profit'
    doc += ['-'*ls,
            'K E Y   P E R F O R M A N C E   I N D I C A T O R S'.center(ls),
